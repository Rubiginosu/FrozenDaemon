...
func (s *ServerLocal)prepareVirtualEnv(){
	...
	if _,err := os.Stat(lfs + "/dev/console");err != nil{
		...
		AutoRunCmdAndOutputErr(cmdMkNodConsole,"make virtual node /dev/console")
	}
	...
	if _,err := os.Stat(lfs + "/dev/null");err != nil{
		...
		AutoRunCmdAndOutputErr(cmdMkNodNull,"make virtual node /dev/null")
	}
	...
	if _,err := os.Stat(lfs + "/dev/core");err != nil{
		...
		AutoRunCmdAndOutputErr(cmdMountDev,"mount virtual file system dev")
	}
	...
	if _,err := os.Stat(lfs + "/dev/pts");err != nil{
		...
		AutoRunCmdAndOutputErr(cmd,"mount devpts")
	}
	// proc
	if _,err := os.Stat(lfs + "/proc/cpuinfo");err != nil{
		...
		AutoRunCmdAndOutputErr(cmd,"mount proc")
	}
	...
	if _,err := os.Stat(lfs + "/sys/fs");err != nil{
		...
		AutoRunCmdAndOutputErr(cmd,"mount sys")
	}
